...
/**
 * Vendored from React Bits (Topography, TypeScript variant).
 *
 * Local change: the render loop is skipped entirely under
 * prefers-reduced-motion — a single frame is drawn so the contours are still
 * there, they just stop breathing.
 */
...
const Topography: React.FC<TopographyProps> = ({
  ...
  useEffect(() => {
    ...
    // LOCAL CHANGE: honour the OS "reduce motion" setting. One frame is still
    // drawn below, so the section keeps its texture without any animation.
    const reduceMotion =
    ...
    // LOCAL CHANGE: seed the control points and draw once before any animation.
    // Left at zero the field is uniformly flat, so the first paint — and every
    // paint under reduced motion or a throttled tab — would be blank.
    for (let g = 0; g < 4; g++) {
      const arr = ctrlArrays[g];
      const idx = CTRL_INDICES[g];
      for (let j = 0; j < 4; j++) {
        arr[j] = (program.uniforms.uMorphAmount.value as number) * Math.sin(idx[j]);
      }
    }
    renderer.render({ scene: mesh });
